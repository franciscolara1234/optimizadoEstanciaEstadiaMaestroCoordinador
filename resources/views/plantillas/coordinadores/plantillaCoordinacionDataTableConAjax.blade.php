    ...
    <script>
        ...
        $(document).ready(function(){
            $('#usuarios').DataTable(
            {
                "searching":true,
            "columns":[
                {data: 'user_proceso.Matricula'},
                {data: 'actions'},
                {data:'tipo_procesos_proceso.nombreProceso'},
                {data:'user_proceso.carrera_user.NombreCarrera'},
               {data: null,
                render: function (data, type, row) 
                    {
                        return data.aa_academico_procesos[0].aa_academico.Nombre+' '+data.aa_academico_procesos[0].aa_academico.APP+' '+data.aa_academico_procesos[0].aa_academico.APM;
                    }
                },
                // {data:'aa_academico_procesos[0].aa_academico.Nombre'+'aa_academico_procesos[0].aa_academico.APP'},
                // {data:'proceso.aa_academico_procesos.aa_academico.Nombre'},
                ...
                render: function (data, type, row) 
                    {
                        ...
                        if (data.calificaciones_proceso.length === 0) {
                                return '<div style="color:#A28614">Sin Asignar</div>';
                        }else if(data.calificaciones_proceso[0].cal_final >= 70) {
                ...
                {data:'periodo_proceso.Periodo'},
                
            ],
                processing: true,
                // serverSide:true,
                responsive: true,
                ajax: "{{route('dataTableCoordinacion', $procesoElegido)}}",
                dataType: 'json',
